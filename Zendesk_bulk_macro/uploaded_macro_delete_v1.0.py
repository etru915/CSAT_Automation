...
def delete_macro(start_cnt) :
    ...
    for i in df['macro_title'][start_cnt:end_cnt]:
        try:
            # blocker_check()
            # 매크로 제목 입력
            driver.find_element_by_xpath('/html/body/div/div/section/div[1]/div[1]/input').send_keys(str(df['macro_title'][work_cnt]))
            ...
            try:
                ...
                # 매크로 항목 메뉴 내 삭제 버튼 클릭
                time.sleep(random.randint(1,5))
                driver.find_element_by_xpath('/html/body/div/div/section/div[5]/table/tbody/tr/td[7]/div/ul/li[4]').click()

                # 매크로 삭제 메시지에 확인 버튼 클릭

                time.sleep(random.randint(3, 5))
                driver.switch_to.parent_frame()
                time.sleep(random.randint(3, 5))
                driver.find_element_by_xpath('/html/body/div[3]/div/div[3]/span[2]/button').click()
                time.sleep(random.randint(3, 5))
                try:
                    # x 버튼 클릭하여 제목 삭제
                    driver.switch_to.frame('zendesk-clean-admin-center')
