...
def zen_edit_access_user(start_cnt):
    ...
    while start_cnt <= end_cnt:
        try:
            ...
            driver.switch_to.frame('zendesk-clean-admin')
            print('작업 1')
# '/html/body/div[1]/div/section/form/div[1]/fieldset[2]/div[2]/div/div/div[1]'
# ''
            ...
            for i in action_list:
                if i.text == '댓글/설명':
                    comment_text = driver.find_element_by_xpath('/html/body/div[1]/div/section/form/div[1]/fieldset[2]/div[2]/div/div/div['
                                                                  + str(cnt_1) + ']/div[2]/div/div[1]/div/div[1]/div[1]/p')
                    print('작업 3')
                    # 댓글/설명에서 전체 내용 삭제
                    comment_text.click()
                    time.sleep(1)
                    print('작업 4')
                    comment_text.send_keys(Keys.CONTROL , 'a')
                    time.sleep(1)
                    print('작업 5')
                    comment_text.send_keys(Keys.DELETE)
                    time.sleep(1)
                    print('작업 6')
                    # 댓글/설명에서 새로운 내용 입력
                    html_text = df['comment_value_html'][start_cnt]
                    print('작업 7')
                    ...
                    print('작업 9')
                    time.sleep(1)
                    driver.execute_script("arguments[0].innerHTML = '" + str(html_text) + "';", elm)
                    print('작업 10')
                    break
                else:
                    print(str(cnt_1),"---",i.text)
