# 2021-09-07
# 등록된 매크로에서 댓글 모드 False->false로 수정하기
...
def zen_edit_access_user(start_cnt):
    ...
    def retry_macro_access():
        ...
        time.sleep(30)
        print("젠데스크 주소 입력 전 : " + driver.window_handles)
        locate = "https://coupangcustomersupport.zendesk.com/agent/admin/macros/" + str(df["macro_id"][start_cnt])
        driver.get(locate)
    ...
    def blocker_check():
        try:
            ...
            if ip_blocker == 'Your IP address is not permitted.':
                ...
                driver.get(locate1)
                time.sleep(30)
                locate2 = "https://coupangcustomersupport.zendesk.com/agent/admin/macros/" + str(df["macro_id"][start_cnt])
                driver.get(locate2)
                time.sleep(10)
                # driver.switch_to.window(driver.window_handles[0])
                logger.info(f'{start_cnt} 번째 작업 중 IP 차단 해제 완료. ')
    ...
    while start_cnt <= end_cnt:
        try:
            locate = "https://coupangcustomersupport.zendesk.com/agent/admin/macros/" + str(int(df["macro_id"][start_cnt]))
            driver.get(locate)
            time.sleep(5)
            blocker_check()
            driver.switch_to.frame('zendesk-clean-admin')
            ...
            for i in action_list:
                if i.text == '댓글 모드':
                    driver.find_element_by_xpath('/html/body/div[1]/div/section/form/div[1]/fieldset[2]/div[2]/div/div/div['+str(j)+']/div[2]/div/button').click()
                    driver.find_element_by_xpath('/html/body/div[1]/div/section/form/div[1]/fieldset[2]/div[2]/div/div/div['+str(j)+']/div[2]/div/input').send_keys('비공개\n')
                    break
                else:
                    j += 1
            ...
            # time.sleep(0.5)
            macro_id = str(int(df["macro_id"][start_cnt]))
