# 2021-09-08
# 등록된 매크로에서 댓글/설명 부분의 일부 내용 삭제
# <본 가이드는 필수 확인 사항입니다. 그 외 내용은 지식을 참고해 주세요.> 내용만 제거
...
def zen_edit_access_user(start_cnt):
    ...
    while start_cnt <= end_cnt:
        try:
            locate = "https://coupangcustomersupport.zendesk.com/agent/admin/macros/" + str(int(df["id"][start_cnt]))
            ...
            for i in action_list:
                if i.text == '댓글/설명':
                    comment_texts = driver.find_elements_by_xpath('/html/body/div[1]/div/section/form/div[1]/fieldset[2]/div[2]/div/div/div['+str(cnt_1) +']/div[2]/div/div[1]/div/div[1]/div[1]/p')
                    # print(str(cnt_1), "---", i.text)
                    cnt_2 = 1
                    for k in comment_texts:
                        # print(str(cnt_2),"--> ", k.text)
                        compare_text = "<본 가이드는 필수 확인 사항입니다. 그 외 내용은 지식을 참고해 주세요.>"
                        if k.text == compare_text:
                            finded_text = driver.find_element_by_xpath('/html/body/div[1]/div/section/form/div[1]/fieldset[2]/div[2]/div/div/div[' + str(cnt_1) + ']/div[2]/div/div[1]/div/div[1]/div[1]/p['+str(cnt_2)+']')
                            driver.execute_script("arguments[0].remove();", finded_text)
                            # print(str(cnt_2), "--> ", "제거 완료")
                            break
                        else:
                            cnt_2 += 1
                    break
